        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            for (int x = -half_patch_size; x < half_patch_size; x++)
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    if (inverse == false) {
                        // Forward Jacobian
                        J[0] = (GetPixelValue(img2,kp.pt.x + x + 1 + dx,kp.pt.y + y + dy) - GetPixelValue(img2, kp.pt.x + x - 1 + dx, kp.pt.y + y + dy))/2;
                        J[1] = (GetPixelValue(img2,kp.pt.x + x + dx,kp.pt.y + y + 1 + dy) - GetPixelValue(img2, kp.pt.x + x + dx, kp.pt.y + y - 1 + dy))/2;
...
void OpticalFlowMultiLevel(
    ...
    for (int i = 0; i < 3; i++) {
        ...
        cv::pyrDown(tmp1,py_prev,Size(tmp1.cols/2,tmp1.rows/2));
        tmp1 = py_prev;
        pyr1.push_back(py_prev);

        cv::pyrDown(tmp2,py_curr,Size(tmp2.cols/2,tmp2.rows/2));
        tmp2 = py_curr;
    ...
    for(int level = pyramids - 1  ; level >= 0; level--)
    {
        ...
        {
            kp1_tmp.pt= kp1[j].pt * scales[level];
            kp1_single.push_back(kp1_tmp);
        }

        OpticalFlowSingleLevel(pyr1[level],pyr2[level], kp1_single,kp2, success, inverse);
