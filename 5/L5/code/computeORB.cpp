...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        float m01 = 0, m10 = 0;
        auto u = int(kp.pt.x), v = int(kp.pt.y);
        if( (u >= half_patch_size) && (u < image.cols - half_patch_size) && (v >= half_patch_size)
            && (u < image.rows - half_patch_size) ){
            for( int i = -half_patch_size; i < half_patch_size ; i++){
                for(int j = -half_patch_size; j < half_patch_size; j++ ){
                    m10 += j * image.at<uchar>(v+i, u+j);
                    m01 += i * image.at<uchar>(v+i, u+j);
                }
            }
        }
        kp.angle = float(std::atan2(m01, m10)*180/pi);
        // END YOUR CODE HERE
    }
    return;
}
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        float up, vp, uq, vq,up_, vp_, uq_, vq_;
        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            //d[i] = 0;  // if kp goes outside, set d.clear()
            float angle = float(kp.angle * pi / 180);
            float c = std::cos(angle), s = std::sin(angle);

            up = c * ORB_pattern[4 * i] - s * ORB_pattern[4 * i + 1];
            vp = c * ORB_pattern[4 * i] + s * ORB_pattern[4 * i + 1];
            uq = c * ORB_pattern[4 * i + 2] - s * ORB_pattern[4 * i + 3];
            vq = c * ORB_pattern[4 * i + 2] + s * ORB_pattern[4 * i + 3];//先旋转
            ...
            if (image.at<uchar> (up_, vp_) > image.at<uchar>(uq_, vq_))
                d[i] = 0;
            else
                d[i] = 1;
        }
        if (up_ < 0 || up_ > image.cols || vp_ < 0 || vp_ > image.cols
            || uq_ < 0 || uq_ > image.cols || vq_ < 0 || vq_ > image.rows)
            d.clear();
...
#define N 256
    ...
    for(int i = 0; i < desc1.size(); i++ ) {
        ...
        for(int j = 0; j < desc2.size(); j++){
            ...
            if(distance < dis_min) dis_min = distance;

            v_c.push_back(distance);
        }
        auto min = min_element(v_c.begin(), v_c.end());
        match.queryIdx = i;match.trainIdx = int(min - v_c.begin());
        match.distance = *min;
        matches1.push_back(match);
        v_c.clear();
    }
    for(int i = 0; i < matches1.size(); i++) {
            if (matches1[i].distance <= max(2*dis_min, 50)) {
